...
export default function Slider6() {
    ...
    return (
        <section ref={sectionRef} className="w-full px-12 pt-[32px] pb-[48px] relative z-10 bg-[rgba(247,248,247,0.70)] text-[#111111]">
                ...
                {faqItems.map((item) => (
                    <div key={item.id} className="group bg-[#f7f8f7] border-2 border-[#111111] rounded-2xl w-full transition-all duration-300">
                        <div className="flex flex-row items-center justify-between px-10 py-[18px] w-full cursor-pointer" onClick={() => toggleFAQ(item.id)}>
                            <div className="text-[24px] leading-[32px] text-nowrap">{item.question}</div>
                        ...
                        <div className={`overflow-hidden transition-all duration-300 ease-in-out ${item.isExpanded && item.answer ? "max-h-[500px] opacity-100" : "max-h-0 opacity-0"}`}>
                            {item.answer && (
